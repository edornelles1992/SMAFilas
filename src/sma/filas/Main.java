...
public class Main {
	...
	private static void executaSimulador(ArrayList<Fila> filas) throws FileNotFoundException {
		...
		System.out.println("EXECUTANDO SIMULA��ES...");
		for (int i = 0; i < qtdSimulacoes; i++) {
			System.out.println();
			System.out.print("========= EXECU��O NUMERO: " + (i + 1) + " ======== SEED: " + seedsCarregadas[i]);
	...
	private static void calculaMediaExecucoes(ArrayList<Fila> filasProcessadas) {
		System.out.println();
		System.out.println();
		System.out.println("SIMULA��ES ENCERRADAS...");
		System.out.println();
		System.out.println("====MEDIA DAS EXECU��ES========");
		System.out.println();
		System.out.println();
		...
		System.out.println();
		System.out.println("M�dia de tempo das execu��es: " + String.format("%.2f", totalTempoMedio));
		System.out.println();
	}

	// M�todo para teste de uma execu��o isolada
	private static void informaResultadoExecucao(ArrayList<Fila> filas, int nroSimulacao) {
		System.out.println("=============RESULTADOS===============");
		System.out.println("Gerou " + 100000 + " Aleat�rios!! FIM!!!");
		...
		for (int x = 0; x < filas.size(); x++) {
			System.out.println();
			System.out.println("Fila " + (x + 1) + " Simula��o " + (nroSimulacao + 1));
	...
	private static void displayDadosRecebidos(ArrayList<Fila> filas) {
		...
		for (int i = 0; i < filas.size(); i++) {
			...
			if (i == 0) {
				System.out.println(
						"CHEGADA: " + filas.get(i).tempoChegadaMinimo + " at� " + filas.get(i).tempoChegadaMaximo);
			}
			System.out.println("TEMPO ATENDIMENTO: " + filas.get(i).tempoAtendimentoMinimo + " at� "
					+ filas.get(i).tempoAtendimentoMaximo);
			System.out.println("NRO ALEATORIOS POR SIMULA��O: "+qtdAleatorios);
			System.out.println("==========================");
		}
	}
	...
	private static ArrayList<Fila> lerDadosFila() throws FileNotFoundException {
		...
		while (in.hasNextLine()) {
			...
			if (comando.contains("SEEDS")) { // linha que corresponde a seeds
				...
				continue;
			} else if (comando.contains("FIM_FILA")) { // fim da fila prepara dados para pr�xima, se houver.
				cont = 0;
